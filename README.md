...
Henter inn verdien lagret i `activityText` og `selected`, og bruker disse til å opprette et nytt element i `listOfActivities`. Kjører også et kall til `_storeData()` for å lagre endringene som er gjort i listen, og et kall til `updateMarks()` for å oppdatere markeringene i kalenderen.
...
Var litt vanskelig å finne ut hvordan man skulle oppdatere `markedDates` siden elementene i arrayet er objekter. Brukte derfor forslagene herfra for å finne en løsning for denne delen: https://github.com/wix/react-native-calendars/issues/160
...
Alle event handler callbacks kalles med `Calendar`-objekter på formen nedenfor (hentet fra deres dokumentasjon). Er slike objekter vi bruker i `onPress`-funksjonen i `CalendarList`-komponenten for å hente ut `datestring` for datoen som er blitt trykket på. 
```javascript
{
...
Vi valgte å bruke `@expo/vector-icons` siden denne allerede er innstallert når man har satt opp et expo-prosjekt med `expo init --`. Den bygger på `react-native-vector-icons`, slik at vi hadde en rekke ikoner tilgjengelig. 
